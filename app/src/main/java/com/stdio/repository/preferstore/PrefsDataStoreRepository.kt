...
class PrefsDataStoreRepositoryImpl @Inject constructor(
    ...
    override suspend fun setSignInAccount(signInAccount: SignInAccount?) {
        if (signInAccount == null) clear(KEY_SIGN_IN_ACCOUNT)
        else setObject(KEY_SIGN_IN_ACCOUNT, signInAccount)
    }
    ...
    override suspend fun setUserInfo(userInfo: User?) {
        if (userInfo == null) clear(KEY_USER_INFO)
        else setObject(KEY_USER_INFO, userInfo)
    }
